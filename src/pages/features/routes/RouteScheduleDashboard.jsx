...
const RouteScheduleDashboard = () => {
    ...
    const handleSaveRoute = async (route) => {
        try {
            ...
            if (!routeId) {
                throw new Error('Route ID is missing. Cannot save route.')
            }
            ...
            if (editingRoute) {
                ...
                setRoutes(prev => prev.map(item => getRouteId(item) === routeId ? route : item))
                ...
                if (!newRouteId) {
                    throw new Error('Created route has no ID in response')
                }
            ...
            console.error('❌ Error saving route:', {
                message: err.message,
                route,
                editingRoute: editingRoute?.routeID ?? editingRoute?.id,
                stack: err.stack
            })
